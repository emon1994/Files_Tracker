            ...
            <div class="flex items-center justify-center p-12">
                <div class="mx-auto w-full max-w-[1000px] rounded-md p-10 bg-gradient-to-r from-blue-800 to-indigo-900">
                    ...
                    <div class="flex justify-between items-center border-b">
                        <div>
                            <a href="{{ route('submit-file') }}"
                                class="bg-gradient-to-r from-red-500 to-pink-500 text-white px-3 py-2 rounded">Add Files</a>
                        </div>
                        <div>
                            <p
                                class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-rose-600 via-red-400 to-orange-500 flex items-center justify-center">
                                FILES</p>
                        </div>

                        {{-- <div>
                            <form action="{{ route('filter.files.by.country') }}" method="GET">
                                <div class="mb-3">
                                    <select name="country" id="country"
                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-2 px-10 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                                        onchange="this.form.submit()">
                                        <option value="" disabled selected>Country</option>
                                        @foreach ($countries as $countryOption)
                                            <option value="{{ $countryOption->country }}"
                                                {{ $country == $countryOption->country ? 'selected' : '' }}>
                        ...
                        <div >
                            <form action="{{ route('filter.files.by.country') }}" method="GET" class="flex gap-2">
                                <div class="mb-3">
                                    <select name="country" id="country"
                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-2 px-10 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
                                        onchange="this.form.submit()">
                                ...
                                <div>
                                    <!-- Hidden input to trigger export action -->
                                    ...
                                    <button type="button"
                                        ...
                                        onclick="document.getElementById('export').value = 1; this.form.submit();">
                                        Export Data
                                    </button>
                                </div>
                            </form>
                    ...
                    @foreach ($codes as $code)
                        @if (isset($files[$code->code]))
                            <div class="mb-3">
                                <p class="text-md font-bold text-white"> <span class="text-sm">{{ $code->code }}</span>
                                </p>
                                        ...
                                        @foreach ($files[$code->code] as $file)
                                            <tr>
                                                ...
                                                @if ($file->status == 'migrated')
                                                    <td class="td-class" style="color: rgb(255, 47, 47);">
                                                        {{ $file->status }}</td>
                                                @else
                    ...
                    <div class="flex justify-end text-white">
                        {{ $codes->appends(['country' => request('country')])->links() }}
                    </div>
                </div>
            </div>
        </div>
